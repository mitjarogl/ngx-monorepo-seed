  ...
  getScreenSizeType(): Observable<ScreenSizeTypeEnum> {
    return combineLatest(
      this.breakpointObserver
        ...
        .pipe(map(val => (val.matches ? ScreenSizeTypeEnum.MOBILE : null))),
      this.breakpointObserver
        .observe([Breakpoints.Tablet])
        .pipe(map(val => (val.matches ? ScreenSizeTypeEnum.DESKTOP : null))),
      this.breakpointObserver
        .observe([Breakpoints.Web])
        .pipe(map(val => (val.matches ? ScreenSizeTypeEnum.DESKTOP : null)))
    ).pipe(map(values => values.find(value => value !== null)));
  }
