...
export class ImageDropFormComponent implements OnInit, OnChanges, OnDestroy {
  ...
  imageValueChangeSubs: Subscription;
  ...
  listenToChanges() {
    this.imageValueChangeSubs = this.form.get('image').valueChanges.pipe(debounceTime(200)).subscribe((formData) => {
  ...
  onFileChange(files) {
    ...
    if (files && files.length > 0) {
      ...
      reader.onload = () => {
        ...
        this.form.get('image').setValue(reader.result.toString().split(',')[1]);
      };
    }
  }
