...
export function resetStore(reducer: ActionReducer<any>): ActionReducer<any> {
  return (state, action) => {
    return reducer(action.type === AuthActionTypes.LOGOUT ? undefined : state, action);
  };
}
...
export const metaReducers: MetaReducer<AppState>[] = [debug, localStorageSyncReducer, resetStore];
export const metaProdsReducers: MetaReducer<AppState>[] = [localStorageSyncReducer, resetStore];
